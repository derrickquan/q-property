...
/** Permanent sample properties (read-only) */
...
export default function PropertiesPage() {
  ...
  // Load once and prepend mocks (use fallback arg!)
  useEffect(() => {
    const saved = load<Property[]>(STORAGE_KEY, []);
    const filteredSaved = saved.filter((s) => !mockIds.has(s.id));
    setItems([...MOCKS, ...filteredSaved]);
  }, []);

  // Persist only non-mock items
  useEffect(() => {
    const nonMock = items.filter((p) => !mockIds.has(p.id));
    save<Property[]>(STORAGE_KEY, nonMock);
  }, [items]);
  ...
  function onEdit(p: Property) {
    if (mockIds.has(p.id)) {
      alert("Sample properties are read-only.");
      return;
    }
    setEditing({ ...p });
    setIsOpen(true);
  ...
  function onDelete(id: string) {
    if (mockIds.has(id)) {
      alert("Sample properties cannot be deleted.");
      return;
    }
          ...
          <table className="min-w-full text-sm">
            ...
            <tbody>
              {filtered.length === 0 ? (
                ...
                filtered.map((p) => {
                  const isMock = mockIds.has(p.id);
                  return (
                    <tr key={p.id} className="border-t border-slate-200">
                      <Td className="font-medium">
                        {p.name}{" "}
                        {isMock && (
                          <span className="ml-2 text-[10px] uppercase tracking-wide rounded-full bg-slate-100 text-slate-600 px-2 py-0.5">
                            Sample
                          </span>
                        )}
                      </Td>
                      ...
                      <Td className="max-w-[240px] truncate" title={p.notes ?? ""}>
                        {p.notes ?? "—"}
                      </Td>
                      <Td className="text-right pr-4">
                        <div className="inline-flex gap-2">
                          <button
                            ...
                            disabled={isMock}
                            title={isMock ? "Sample properties are read-only" : "Edit"}
                          >
                            Edit
                          </button>
                          <button
                            ...
                            disabled={isMock}
                            title={isMock ? "Sample properties are read-only" : "Delete"}
                          >
                            Delete
                          </button>
                        </div>
            ...
            <form onSubmit={onSubmit} className="p-4 grid gap-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <label className="grid gap-1">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setEditing({ ...editing, unitCount: Number(e.target.value || 0) })
                    }
                    className="rounded-lg border border-slate-300 px-3 py-2 outline-none focus:ring-2 focus:ring-blue-500"
                    required
                  />
              ...
              <label className="grid gap-1">
                ...
                <textarea
                  ...
                  className="rounded-lg border border-slate-300 px-3 py-2 outline-none focus:ring-2 focus:ring-blue-500"
                  placeholder="e.g., Rent range, parking details, HOA, etc."
                />
              </label>
